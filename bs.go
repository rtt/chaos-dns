...
func decodeBuffer(b *bytePacketBuffer) (*Header, *Query, *Response, error) {
	...
	if header.response {
		// skip over the next two bytes, they are the name indexes
	...
	return header, query, response, nil
}
...
func main() {
	...
	//buf := newBytePacketBufferFromFile("response_packet_google.com")
	buf := newBytePacketBufferFromFile("response_packet_vogue.co.uk")
	dumpQuery(buf)

	fmt.Println("-----")

	// todo: handle the error
	header, query, response, _ := decodeBuffer(buf)

	fmt.Println("Query id: ", header.queryIdHex())
	fmt.Println("Is response:", header.response)
	fmt.Println("Opcode:", header.opcode)
	fmt.Println("AA Authoritative:", header.isAuthoritative)
	fmt.Println("TC Truncated:", header.isTruncatedMessage)
	fmt.Println("RD Recursion desired:", header.recursionDesired)
	fmt.Println("RD Recursion available:", header.recursionAvailable)
	fmt.Println("DNSSEC:", header.dnsSec)
	fmt.Println("Res code:", header.resCode, header.resCodeStr())
	fmt.Println("Query count:", header.questions)
	fmt.Println("Answer count:", header.answers)
	fmt.Println("Authority count:", header.authoritative)
	fmt.Println("Additional count:", header.additional)

	if query != nil {
		fmt.Println("Query:", query.name)
		fmt.Println("Query type:", query.rTypeStr())
		fmt.Println("Query class: IN", query.class)
	}

	if response != nil {
		fmt.Println("Response record class:", response.recordClass)
		fmt.Println("Response record type:", response.recordType, getRecordTypeName(response.recordType))
		fmt.Println("TTL: ", response.ttl)
		fmt.Println(response.asStr())
	}
}
